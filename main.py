...
app = FastAPI(
    title="School LawBot (최적 실무형)",
    description="정확히 일치하는 조문만 기본 제공, 옵션으로 유사/부분일치 조문 본문까지, 안내/메타/UX/캐싱/AI 모두 포함",
    version="2024.06.01"
)
...
def extract_article(xml_text, article_no_raw, include_similar=False):
    """
    - 완전 일치 조문이 있으면 그 본문/구조 제공
    - 옵션으로 유사(부분일치) 조문도 허용할 경우, 안내와 함께 본문/구조 제공
    - 없으면 안내/유사조문 목록/링크만 반환
    """
    try:
        ...
        for article in articles:
            if article.get("조문번호", "") == article_no_raw:
                ...
                return body, available, 구조화, "정확히 일치"
        # 2. 옵션이 켜져 있으면 부분일치(포함) 반환
        if include_similar:
            for article in articles:
                if article_no_raw in article.get("조문번호", ""):
                    body = article.get("조문내용", "")
                    구조화 = split_article_text_to_structure(body)
                    return "[유사조문] " + body, available, 구조화, "유사조문"
        # 3. 아예 없으면 본문 없음
        return "", available, None, "없음"
    except Exception as e:
        return f"파싱 오류: {e}", [], None, "오류"
...
def law(
    ...
    include_similar: bool = Query(False, description="유사/부분일치 조문까지 허용할지"),  # 🌟 옵션
    ...
    cache_key = f"{law_name}:{fixed_article_no}:{include_similar}"
    ...
    body, available, 구조화, match_type = extract_article(res.text, fixed_article_no, include_similar)
    found = match_type in ["정확히 일치", "유사조문"]
    direct_link = make_article_link(law_name, fixed_article_no)
    summary = body[:100].replace('\n', ' ') + ("..." if len(body) > 100 else "")

    # 안내 메시지 분기
    if match_type == "정확히 일치":
        message = "정상적으로 조회되었습니다."
    elif match_type == "유사조문":
        message = (
            f"정확히 일치하는 '{law_name} {fixed_article_no}' 조문은 존재하지 않습니다. "
            f"아래는 가장 유사한 조문({[a for a in available if fixed_article_no in a][0]})의 내용입니다."
        )
    elif match_type == "없음":
        message = (
            f"'{law_name} {fixed_article_no}'에 해당하는 조문이 존재하지 않습니다."
            + (f"\n📌 현재 {law_name}에는 다음과 같은 유사 조문이 존재합니다:\n- " + "\n- ".join(available) if available else "")
            + f"\n사이트에서 직접 확인: {direct_link}"
        )
    else:  # 오류
        message = "조문 데이터 파싱 오류가 발생했습니다."
...
def root():
    return {
        "message": "School LawBot API (최적 실무형) Running",
        "guide": user_guide(),
        "features": [
            "기본: 정확히 일치하는 조문만 본문/구조 제공",
            "옵션: 유사(부분일치) 조문 본문도 제공",
            "유사/AI/실험 등은 분리·옵션화, 안내·메타·UX 강화",
            "모바일/PC viewType, 캐싱, 프라이버시 등 유지"
        ]
    }
